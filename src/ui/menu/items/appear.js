...
async function createLogoManagerTempMenu(logoType) {
  ...
  const defaultLogoPreview = logoType === SiteType.Forum ?
  ...
  const el = addTemporaryMenuSection(
    [
      new Container(
        ...
        'Выберите логотип',
        `Выберите логотип, который у вас будет отображаться как логотип ${logoType === SiteType.Forum ? 'форума' : 'маркета'}.`
      ).createElement()
    ]
  )
...
const getAppearItems = async () => {
  ...
  const appearSection = new Section('LZTUpInfoSection', { direction: SectionDirection.Column, hidden: false })
    .addSectionItem('Логотип', 'Выберите логотип для форума из списка', 'far fa-comments', 'LZTUpLogoManager', { onClick: async () => await createLogoManagerTempMenu(SiteType.Forum), rightArrow: true })
    .addSectionItem('Логотип маркета', 'Выберите логотип для маркета из списка', 'far fa-shopping-cart', 'LZTUpMarketLogoManager', { onClick: async () => await createLogoManagerTempMenu(SiteType.Market), rightArrow: true })

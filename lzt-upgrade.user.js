...
(async function () {
  ...
  if (GM_info?.script?.version)
    Logger.log(`LZT Upgrade version: ${GM_info?.script?.version}`);

  if (typeof GM_getResourceText === "undefined") {
    fetch(
      "https://raw.githubusercontent.com/ilyhalight/lzt-upgrade/master/public/css/style.css",
    ).then(response => response.text().then(styles => GM_addStyle(styles)));
    fetch(
      "https://raw.githubusercontent.com/ilyhalight/lzt-upgrade/master/public/css/coloris.css",
    ).then(response => response.text().then(coloris => GM_addStyle(coloris)));
    fetch(
      "https://raw.githubusercontent.com/ilyhalight/lzt-upgrade/master/public/css/sign.css",
  ...
  let isUniqueDBInited = uniqueStyleDB
    .init()
    .then(value => {
      return value;
    })
  ...
  let isContestsDBInited = await contestsDB
    .init()
    .then(value => {
      return value;
    })
  ...
  let isUsersDBInited = await usersDB
    .init()
    .then(value => {
      return value;
    })
  ...
  let isAppearDBInited = await appearDB
    .init()
    .then(value => {
      return value;
    })
  ...
  let isSettingsDBInited = await settingsDB
    .init()
    .then(value => {
      return value;
    })
  ...
  const START_SCRIPT = async () => {
    ...
    if (/^(Error\s[0-9]{3}|Site\sMaintenance)$/.test($("head title").text())) {
      ...
      if (body.attr("id") !== "LZTUpErrorPage") {
        ...
        body
          .find("article > div")
          .append(
            '<img src="https://i.imgur.com/iVmKDr7.gif" alt="utya_duck_rain" loading="lazy">',
          );
        return "site error";
      }
      return "not auth";
    ...
    // TODO: realize this
    const blockAds =
      "(Native/\\.NET файлов|threads\\/|easyliker\\.ru|niccord\\.ru|vpromotions\\.ru|skysmm\\.ru|VerifTeam|SmmPanelUS\\.com|t\\.me/lztnext|axxishop\\.ru|LIGHTSHOP\\.SU)";
      ...
      if (typeof authorID === "string") {
        if (authorID.includes(",") && authorName.includes(",")) {
          ...
          for (i = 0; i < authorIDArray.length; i++) {
            res += `<a href="https://${currentDomain}/members/${
              authorIDArray[i]
            }">${XenForo.htmlspecialchars(authorNameArray[i])}</a>${
              i === authorIDArray.length - 1 ? "" : ", "
            }`;
          }
          return res;
        } else {
          return `<a href="https://${currentDomain}/members/${authorID}">${XenForo.htmlspecialchars(
            authorName,
          )}</a>`;
        }
    ...
    $(menuBtn).on("click", async function () {
      ...
      async function getUniqueContent() {
        try {
          const uniqueData = await uniqueStyleDB
            .read()
            .then(value => {
              return value;
            })
          ...
          const profileBackgroundEverywhere =
            uniqueData.profileBackgroundEverywhere;
          ...
          return `
              ...
              <div id="LZTUpPreviewContainer" class="previewContainer">
                <div class="avatarBox">
                  <div class="avatarUserBadges">
                    <span id="LZTUpPreviewBadge" class="avatarUserBadge uniq_default badgeDefaultBackground Tooltip" tabindex="0" title="${
                      uniqueData.badgeText
                    }" style="${uniqueData.bannerStyle}"></span>
                  </div>
                ...
                <div class="info">
                  <span id="LZTUpUsernameStyle" class="UsernameStyle bold" style="${
                    uniqueData.nickStyle
                  }">${username}</span>
                  <div class="bannerOrStatus">
                    <em id="LZTUpUserBannerStyle" class="UserBannerStyle userBanner" style="${
                      uniqueData.bannerStyle
                    }">${uniqueData.bannerText}</em>
                  </div>
                </div>
              </div>
            </div>
            ...
            <input id="LZTUpBannerText" type="text" maxlength="24" class="textCtrl" value="${XenForo.htmlspecialchars(
              bannerText,
            )}">
            ...
            <input id="LZTUpBadgeText" maxlength="24" type="text" class="textCtrl" value="${XenForo.htmlspecialchars(
              badgeText,
            )}">
  
            <div id="LZTUpModalHeading" class="textHeading">Фон в профиле</div>
            <input id="LZTUpProfileBackground" type="text" class="textCtrl" placeholder="Ссылка на изображение" value="${XenForo.htmlspecialchars(
              profileBackground,
            )}">
  
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="profile_background_everywhere" ${
                profileBackgroundEverywhere === 1 ? "checked" : ""
              }>
          ...
          Logger.error(
            `Не удалось загрузить меню "Локального уника": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
      ...
      async function getContestsContent() {
        try {
          const contestsData = await contestsDB
            .read()
            .then(value => {
              return value;
            })
          ...
          return `
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="contests_open_ten" ${
                contestsTen === 1 ? "checked" : ""
              }>
              <label for="contests_open_ten">Кнопка "Открыть 10"</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="contests_open_all" ${
                contestsAll === 1 ? "checked" : ""
              }>
      ...
      async function getUsersContent() {
        try {
          ...
          return `
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="show_userid_in_profile" ${
                showUseridInProfile === 1 ? "checked" : ""
              }>
              <label for="show_userid_in_profile">Показывать UserID в профиле пользователя</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="show_fullreg_in_profile" ${
                showFullRegInProfile === 1 ? "checked" : ""
              }>
              <label for="show_fullreg_in_profile">Показывать полную дату регистрации в профиле пользователя</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="disable_show_typing" ${
                disableShowTyping === 1 ? "checked" : ""
              }>
          ...
          Logger.error(
            `Не удалось загрузить меню "Пользователей": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
      ...
      async function getAppearContent() {
        try {
          const appearData = await appearDB
            .read()
            .then(value => {
              return value;
            })
          ...
          return `
            ...
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="hide_unread_article_circle" ${
                hideUnreadArticleCircle === 1 ? "checked" : ""
              }>
              <label for="hide_unread_article_circle">Скрыть значок непрочитанных статей в шапке сайта</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="hide_tags_in_threads" ${
                hideTagsInThreads === 1 ? "checked" : ""
              }>
              <label for="hide_tags_in_threads">Скрыть теги в темах</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="hide_counter_alerts" ${
                hideCounterAlerts === 1 ? "checked" : ""
              }>
              <label for="hide_counter_alerts">Скрыть счётчик уведомлений в навбаре</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="hide_counter_conversations" ${
                hideCounterConversations === 1 ? "checked" : ""
              }>
              <label for="hide_counter_conversations">Скрыть счётчик сообщений в навбаре</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="enable_background_effect" ${
                backgroundEffect === 1 ? "checked" : ""
              }>
              <label for="enable_background_effect">Включить снег (by <a href="/members/576497/" class="muted">Karasu_</a>)</label>
            </div>
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="hide_onlyfans" ${
                hideOnlyfans === 1 ? "checked" : ""
              }>
            ...
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="show_polls_results" ${
                showPollsResults === 1 ? "checked" : ""
              }>
          ...
          Logger.error(
            `Не удалось загрузить меню "Внешнего вида": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
      ...
      async function getAppearLogosContent() {
        try {
          ...
          return `
            <div id="LZTUpModalCell">
              ...
              <div class="LZTUpModalMesh" id="LZTUpModalLogosContainer">
                <div class="LZTUpModalMeshItem ${
                  forumLogo === 0 ? "active" : ""
                }" id="set_default_logo" data-checked="${
                  forumLogo === 0 ? "true" : "false"
                }">
          ...
          Logger.error(
            `Не удалось загрузить меню "Внешнего вида (Логотипы)": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
              Ваша база данных "Внешнего вида" повреждена. Пожалуйста, сбросьте настройки с помощью кнопки ниже.
      ...
      async function getAppearMarketLogosContent() {
        try {
          ...
          return `
            <div id="LZTUpModalCell">
              ...
              <div class="LZTUpModalMesh" id="LZTUpModalMarketLogosContainer">
                <div class="LZTUpModalMeshItem ${
                  marketLogo === 0 ? "active" : ""
                }" id="set_default_marketlogo" data-checked="${
                  marketLogo === 0 ? "true" : "false"
                }">
          ...
          Logger.error(
            `Не удалось загрузить меню "Внешнего вида (Логотипы маркета)": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
              Ваша база данных "Внешнего вида" повреждена. Пожалуйста, сбросьте настройки с помощью кнопки ниже.
      ...
      async function getAppearThemesContent() {
        try {
          ...
          return `
            <div id="LZTUpModalCell">
              <div id="LZTUpModalChecksContainer">
                <input type="checkbox" id="theme_auto_reload" ${
                  themeAutoReload === 1 ? "checked" : ""
                }>
              ...
              <div id="LZTUpModalCell">
                ...
                <div class="LZTUpModalMesh" id="LZTUpModalThemesContainer">
                  <div class="LZTUpModalMeshItem theme ${
                    themeID === 0 ? "active" : "none"
                  }" id="set_default_theme" data-checked="${
                    themeID === 0 ? "true" : "false"
                  }">
          ...
          Logger.error(
            `Не удалось загрузить меню "Внешнего вида (темы)": ${e.message}`,
          );
          return `
            <div id="LZTUpModalComment">
              Ваша база данных "Внешнего вида" повреждена. Пожалуйста, сбросьте настройки с помощью кнопки ниже.
      ...
      async function getUpdatesContent() {
        try {
          const settingsData = await settingsDB
            .read()
            .then(value => {
              return value;
            })
          ...
          return `
            ...
            <div id="LZTUpModalChecksContainer">
              <input type="checkbox" id="check_updates_on_load" ${
                checkUpdatesOnLoad === 1 ? "checked" : ""
              }>
      ...
      availabledGroups.forEach(group => {
        ...
        selectGroups.append(
          `<option value="${group.name}">${group.groupName}</option>`,
        );
      });
      ...
      async function setValueInMesh(target, container) {
        ...
        let currentElement =
          $(target)[0].tagName.toLowerCase() === "div"
            ? $(target)
            : $(target).parent();
      ...
      $.ajax({
        ...
        success: value => {
          if (value && value.length) {
            ...
            value.forEach(logo => {
              if (logo.active === 1) {
                let authorElem = validateAuthors(
                  logo.author_userid,
                  logo.author,
                );
                ...
                $logoSelect.append(`
                <div class="LZTUpModalMeshItem ${
                  forumLogo === logo.uid ? "active" : ""
                }" id="set_${shortName}_logo" data-checked="${
                  forumLogo === logo.uid ? "true" : "false"
                }">
                  <img src="${logo.preview}" alt="logo">
                  <span class="LZTUpModalMeshItemName" data-shortname="${shortName}">${XenForo.htmlspecialchars(
                    logo.name,
                  )}</span>
                ...
                $(document).on(
                  "click",
                  `#set_${shortName}_logo`,
                  async function (event) {
                    await setValueInMesh(
                      event.target,
                      "#LZTUpModalLogosContainer",
                    );
          ...
          if (value && value.length) {
            ...
            value.forEach(logo => {
              if (logo.active === 1) {
                ...
                $logoSelect.append(`
                <div class="LZTUpModalMeshItem ${
                  marketLogo === logo.uid ? "active" : ""
                }" id="set_${shortName}_marketlogo" data-checked="${
                  marketLogo === logo.uid ? "true" : "false"
                }">
                  <img src="${logo.preview}" alt="logo">
                  <span class="LZTUpModalMeshItemName" data-shortname="${shortName}">${XenForo.htmlspecialchars(
                    logo.name,
                ...
                $(document).on(
                  "click",
                  `#set_${shortName}_marketlogo`,
                  async function (event) {
                    await setValueInMesh(
                      event.target,
                      "#LZTUpModalMarketLogosContainer",
                    );
          ...
          if (value && value.length) {
            ...
            value.forEach(theme => {
              if (theme.active === 1) {
                ...
                $themeSelect.append(`
                <div class="LZTUpModalMeshItem theme ${
                  themeID === theme.uid ? "active" : ""
                }" id="set_${shortName}_theme" data-checked="${
                  themeID === theme.uid ? "true" : "false"
                }" style="background:${theme.accent_color}">
                  <span class="LZTUpModalMeshItemName" data-shortname="${shortName}" style="color:${
                    theme.text_color
                  }">${XenForo.htmlspecialchars(theme.name)}</span>
                  <span class="LZTUpModalMeshItemAuthor">${authorElem}</span>
                </div>`);
                $(document).on(
                  "click",
                  `#set_${shortName}_theme`,
                  async function (event) {
                    await setValueInMesh(
                      event.target,
                      "#LZTUpModalThemesContainer",
                    );
                    await appearDB.update({ theme: theme.uid });
                    registerAlert(
                      "Тема изменена. Обновите страницу, чтобы избежать багов с некорректным отображением стилей",
                      5000,
                    );
                    await loadTheme(theme.file);
                    if (themeAutoReload) {
      ...
      $(document).on("click", `#set_default_theme`, async function (event) {
        ...
        registerAlert(
          "Устанаволена стандарная тема. Выполняю перезагрузку страницы...",
          5000,
        );
        await sleep(800);
        window.location.reload();
      ...
      async function createGoBackBtn(callback) {
        lztUpgradeModalMain.prepend(
          $(
            '<button id="LZTUpModalBackButton"><i class="fas fa-long-arrow-left"></i></button>',
          ),
        );
      ...
      $menuTab.toArray().forEach(element => {
        $(element).find("span").text() === "Настройки"
          ? ($settingsTab = $(element))
          : ($mainTab = $(element));
      });
      ...
      if ($menuTab.length) {
        $mainTab.on("click", async () => {
          $($mainTab).find("span").text() === "Главная" &&
          !$mainTab.hasClass("active")
            ? ($settingsTab.removeClass("active"),
              $mainTab.addClass("active"),
              $settingsList.hide(),
              $mainList.show())
        ...
        $settingsTab.on("click", () => {
          $($settingsTab).find("span").text() === "Настройки" &&
        ...
        $(
          "div.LZTUpSubMenu#LZTUpAppearContainer > div.LZTUpModalSection#LZTUpLogoSection",
        ).on("click", async () => {
          await updateSubMenu(
            "appear",
            $appearContainer,
        ...
        $(
          "div.LZTUpSubMenu#LZTUpAppearContainer > div.LZTUpModalSection#LZTUpMarketLogoSection",
        ).on("click", async () => {
          await updateSubMenu(
            "appear",
            $appearContainer,
            $marketLogoSubContainer,
            "Внешний вид — Логотип маркета",
          );
        });

        $(
          "div.LZTUpSubMenu#LZTUpAppearContainer > div.LZTUpModalSection#LZTUpThemesSection",
        ).on("click", async () => {
          await updateSubMenu(
            "appear",
            $appearContainer,
            $themesSubContainer,
            "Внешний вид — Менеджер тем",
          );
        });
    ...
    async function reloadNickStyle() {
      try {
        var uniqueData = await uniqueStyleDB
          .read()
          .then(value => {
            return value;
          })
    ...
    async function sendMessageHandler() {
      const sendButton = $(
        ".sendMessageContainer > button.lzt-fe-se-sendMessageButton",
      );
      ...
      const sendProfileButton = $(
        "#ProfilePoster > .submitUnit > .button.primary",
      );
    ...
    function updateBannerStyle(style, text) {
      ...
      const bannerBtn =
        $(`<em ${bannerStyle} id="LZTUpCustomBanner" itemprop="title"> \
    ...
    async function registerUserBadges(
      bannerStyle = "",
      badgeText = "",
      badgeIcon = "",
    ) {
      for (const el of $.merge(
        $(`.avatarHolder > a.Av${getUserid()}m`),
        $(`.avatarHolder > a.Av${getUserid()}s`),
      )) {
        ...
        if (!$userBadges.length)
          $userBadges = $(
            '<div class="avatarUserBadges" id="LZTUpUserBadges"></div>',
        ...
        $(`<span style="${
          bannerStyle.startsWith(".")
            ? ""
            : XenForo.htmlspecialchars(bannerStyle)
        }" id="LZTUpTooltip" class="avatarUserBadge Tooltip ${
          badgeIcon === ""
            ? "uniq_default"
            : badgeIcon.startsWith(".")
            ? XenForo.htmlspecialchars(badgeIcon.replace(".", "userBanner "))
            : ""
        } ${
          badgeId ? `avatarUserBadge--${badgeId}` : ""
        }" title="${XenForo.htmlspecialchars(badgeText)}" tabindex="0">
          ${
            !badgeIcon.startsWith(".")
              ? `
            <div class="customUniqIcon">
              ${badgeIcon
                .replaceAll(/<[script|style]*>/gi, "<!--")
                .replaceAll(/<\/[script|style]*>/gi, "-->")}
            </div>`
              : ""
          }
        </span>`).appendTo($avatarHolder.find(".avatarUserBadges"));
      }
      return true;
    }
    ...
    async function reloadUserBadges() {
      try {
        ...
        if (typeof uniqueData === "object") {
          ...
          await registerUserBadges(
            uniqueData.bannerStyle,
            uniqueData.badgeText,
            uniqueData.badgeIcon,
          );
    ...
    async function messageClickUsernameHandler() {
      ...
      if (message.attr("data-author") === username) {
        var usernameBtn = $(message).find(
          ".messageInfo > .userText > .item > .username span",
        );
    ...
    async function setUniqIconColor(badgeFill = "", badgeStroke = "") {
      for (const el of $.merge(
        ...
        const $avatarHolder = $(el).parent();
        var $userBadge = $avatarHolder.find(
    ...
    async function getContestsLinks() {
      ...
      $stickyThreads.is(":visible")
        ? $latestsThreads.add($stickyThreads)
        : undefined;
      var links = $latestsThreads
        .find("div.discussionListItem--Wrapper")
    ...
    async function hideThreadsByTag(name = "OnlyFans") {
      ...
      $stickyThreads.is(":visible")
        ? $latestsThreads.add($stickyThreads)
        : undefined;
      var threads = $latestsThreads
        .find("div.discussionListItem--Wrapper")
        .toArray();
      threads.forEach(thread => {
        var threadTags = $(thread)
          .find("a.listBlock.main")
          .find("span.secondRow")
          .find(".threadTitle--prefixGroup")
          .find(".prefix")
          .toArray()
    ...
    async function regOpenContestsBtn(amount = 10) {
      if (await isContestsNode()) {
        ...
        var updateButton = $(
          "div.pageNavLinkGroup > div.linkGroup.SelectionCountContainer > span.UpdateFeedButton.UpdateFeedButtonIcon",
        );
        var getContestsButton = $(
          `<a class="button" id="openContestsButton${XenForo.htmlspecialchars(
            amount,
          )}">Открыть ${
            amount < 100 ? XenForo.htmlspecialchars(amount) : "прогруженные"
          }</a>`,
        ).on("click", async () => {
          updateButton.click();
          await sleep(1000);
            ...
            $(links).map((element, value) => {
              if (element <= amount) {
                ...
                win
                  ? win.focus()
                  : alert(
                      "Разрешите доступ к всплывающим окнам для этого сайта",
                    );
              } else {
                return;
              }
        ...
        updateButton.length
          ? updateButton.parent().prepend(getContestsButton)
          : undefined;
      }
    }
    ...
    async function removeOpenContestsBtn(amount = 10) {
      if (await isContestsNode()) {
        var $oldOpenContestsButton = $(
          `#openContestsButton${XenForo.htmlspecialchars(amount)}`,
        );
        $oldOpenContestsButton.length > 0
          ? $oldOpenContestsButton.remove()
          : null;
      }
    }
    ...
    async function isContestsNode() {
      ...
      return $contestsTitleBar.length > 0 &&
        $contestsTitleBar.attr("title") === "Розыгрыши"
        ? true
        : false;
    }
    ...
    async function contestThreadBlockMove(toTop = true) {
      if (await isContestThread()) {
        ...
        if (toTop) {
          ...
          $contestsThreadBlock
            .css("border-top", "none")
            .css("border-bottom", "1px solid rgb(45, 45, 45)");
        } else {
          $contestsThreadBlock.before($messageContent);
          $contestsThreadBlock
            .css("border-top", "1px solid rgb(45, 45, 45)")
    ...
    async function contestsBtnInBlockMove(toTop = true) {
      if (await isContestThread()) {
        var $contestsThreadBlock = $("div.contestThreadBlock");
        var $participateButton = $contestsThreadBlock.find(
          "a.LztContest--Participate",
        );
        var contestEnded =
          $contestsThreadBlock.find("span.button.contestIsFinished").length > 0
            ? true
            : false;

        if (!$participateButton.length > 0) {
          $participateButton = $contestsThreadBlock.find(
            "span.LztContest--alreadyParticipating",
          );
          if (!$participateButton.length > 0) {
            $participateButton = $contestsThreadBlock.find(
              "span.button.contestIsFinished",
            );
          }
        ...
        if (toTop) {
          // to top
          var $contestsInfoHeading =
            $contestsThreadBlock.find("div.textHeading");
          ...
          $contestCaptcha === undefined
            ? null
            : $participateButton.after($contestCaptcha);
        } else {
          // to default (bottom)
          var marginToFind =
            contestEnded === true
              ? "div.marginBlock:nth-child(7)"
              : "div.marginBlock:nth-child(9)";
          ...
          $contestCaptcha === undefined
            ? null
            : $participateButton.before($contestCaptcha);
        }
      }
    }
    ...
    async function getProfileUserid() {
      if (await isProfilePage()) {
        var marketLink = $(
          "div#page_info_wrap > div.profile_info > div.userContentLinks > a:nth-child(2)",
        ).attr("href");
        var userid = marketLink
          .replace("market/user/", "")
          .replace("/items", "");
        return userid;
      }
    ...
    async function getProfileUsername() {
      if (await isProfilePage()) {
        var username = XenForo.htmlspecialchars(
    ...
    async function addUserIdInProfileInfo() {
      if (await isProfilePage()) {
        ...
        var profileInfoRow1 = $(
          "div#page_info_wrap > div.profile_info > div.pairsJustified > div.clear_fix:nth-child(1)",
        );
        var row = `
    ...
    async function editUserRegInProfileInfo(full = false) {
      if (await isProfilePage()) {
        var $DateTime = $(
          "div#page_info_wrap > div.profile_info > div.pairsJustified > div.clear_fix:nth-child(1) > div.labeled > span.DateTime",
        );
    ...
    async function getContestParticipants(contestBlock) {
      ...
      if (contestBlock.includes("Приняли участие: ")) {
        participants = contestBlock
          .split("Приняли участие: ")[1]
          .split(" пользовател")[0];
    ...
    async function contestsShowWinChance(show = true) {
      if (await isContestThread()) {
        if (!$("#LZTUpWinChance").length && show) {
          ...
          const contestInformationText = $(
            ".contestThreadBlock > .textHeading",
          ).first();
          const winChanceHtml = $(`
    ...
    function updateSiteLogo(site, newStyles) {
      switch (site) {
        case "main" || "forum":
          ...
          break;
        case "market" || "shop":
          $("#lzt-market-logo").length
            ? ($("#lzt-market-logo")[0].style = newStyles)
            : null;
          break;
        default:
          return false;
    ...
    const counterMutationObserver = new MutationObserver(async function (
      mutations,
    ) {
      mutations.forEach(async function (mutation) {
        if (
          mutation.target === $AlertsCounter[0] ||
          mutation.target === $ConversationsCounter[0]
        ) {
          var dbAppearData =
            (await appearDB
              .read()
      ...
      for (let block of $messageList) {
        if ($(block).find(".publicControls").length) {
          ...
          publicControls.forEach(control => {
            if (
              $(control).find(`#LZTUPReportButton[name$=${name}]`).length === 0
            ) {
              let $reportButton = $(
                `<span id = "LZTUPReportButton" name = "${name}">${XenForo.htmlspecialchars(
                  name,
                )}</span>`,
              );
              $reportButton.on("click", async () => {
                ...
                await fetch("posts/" + block.id.split("-")[1] + "/report", {
                  method: "POST",
                  body: formData,
                });
    ...
    async function removeReportBtnInPosts(name) {
      ...
      for (let block in $messageList) {
        const $reportBtn = $(block).find(
          `.publicControls > #LZTUPReportButton[name$=${name}]`,
        );
      ...
      if (dbAppearData && dbAppearData.reportButtonsInPost.length > 0) {
        if (typeof dbAppearData.reportButtonsInPost === "string") {
          ...
          for (const button of buttons) {
            let reportBtn = reportButtonsList.find(
              btn => btn.id === Number(button),
            );
            typeof reportBtn === "object"
              ? await addReportBtnInPosts(reportBtn.name, reportBtn.reason)
              : undefined;
          }
        }
      }
    ...
    async function getPollResults() {
      ...
      response = await fetch(
        `https://${url.hostname}/${url.pathname}poll/results?=&_xfRequestUri=${url.pathname}&_xfNoRedirect=1&_xfToken=${XenForo._csrfToken}&_xfResponseType=json`,
      );
      if (response.status === 200) {
        let result = await response.json();
        if (result.templateHtml) {
          let $pollHtml = $(result.templateHtml);
          let pollPercentage = $pollHtml
            .find(".pollResults > .pollResult > .percentage")
            .toArray()
            .map(el => el.innerText.trim());
          let pollCounts = $pollHtml
            .find(
              ".pollResults > .pollResult > .barCell > .barContainer > .count",
            )
            .toArray()
            .map(el => el.innerText.trim());
          for (
            let i = 0;
            i < pollPercentage.length && i < pollCounts.length;
            i++
          ) {
          ...
          Logger.error(
            `LZT Upgrade: Не удалось найти тело результатов голосования. Статус: ${response.status}`,
          );
        }
      } else {
        Logger.error(
          `LZT Upgrade: Не удалось загрузить результаты голосования. Статус: ${response.status}`,
        );
      }
    ...
    function removeExtraClasses(element, defaultClasses) {
      ...
      elementClassList.forEach(selectedClass => {
        if (!defaultClasses.includes(selectedClass))
    ...
    async function addNoticeMark(name) {
      Array.from($(".username span")).forEach(item => {
        if ($(item).text() === username) {
          ...
          if (
            !headerContent.parent()[0].classList.contains("navLink") &&
            !headerContent
              .parent()
              .parent()[0]
              .classList.contains("fr-element") &&
            !headerContent.parent().find(`span.${name}`).length
          ) {
            let tooltipText = "";
            if (name === "scamNotice") {
              tooltipText =
                "Внимание! На данного пользователя недавно жаловались в связи с мошенничеством. Советуем вам быть осторожным и пользоваться гарантом, имея дело с ним.";
            } else if (name === "vpnNotice") {
              tooltipText =
                "Внимание! Данный пользователь использует VPN, будьте осторожны.";
            }
            const notice = $(
              `<span id="LZTUpCustomNotice" class="${name} Tooltip" title="${tooltipText}"></span>`,
            );
      ...
      if (dbUniqueStyleData && dbUniqueStyleData?.noticesMarks?.length) {
        if (typeof dbUniqueStyleData.noticesMarks === "string") {
          ...
          for (const noticeMark of noticesMarks) {
            let noticeMarkSpan = noticesList.find(
              notice => notice.id === Number(noticeMark),
            );
            typeof noticeMarkSpan === "object"
              ? await addNoticeMark(noticeMarkSpan.name)
              : undefined;
          }
        }
      }
    ...
    async function updateBackground() {
      ...
      if (
        dbUniqueStyleData &&
        dbUniqueStyleData.profileBackground !== undefined
      ) {
        if (
          (await getProfileUrl()) ===
            $("#AccountMenu > ul:nth-child(1) > li:nth-child(1) > a").attr(
              "href",
            ) ||
          typeof dbUniqueStyleData.profileBackgroundEverywhere === "number"
        ) {
          if (
            (await isProfilePage()) &&
            (await getProfileUrl()) !==
          ...
          const body = $("body");
          if (
            dbUniqueStyleData.profileBackgroundEverywhere === 0 &&
            dbUniqueStyleData.profileBackground.length
          ) {
            body.attr("id", "");
          ...
          if (dbUniqueStyleData.profileBackground.length) {
            ...
            body.css(
              "background-image",
              `linear-gradient(rgba(54, 54, 54, 0.85), rgba(54, 54, 54, 0.85)), url(${dbUniqueStyleData.profileBackground})`,
            );
          } else {
            body.attr("id", "");
            body.css("background-image", "");
    ...
    async function disableShowTypingExecute() {
      XenForo.hasOwnProperty("threadNotify") &&
      XenForo.threadNotify.hasOwnProperty("shareTypingActivity")
        ? (XenForo.threadNotify.shareTypingActivity = 0)
        : null;
      XenForo.hasOwnProperty("ChatboxRTC") &&
      XenForo.ChatboxRTC.hasOwnProperty("Start")
        ? (XenForo.ChatboxRTC.Start.prototype.sendTypingMessage = () => {
            return;
          })
        : null;
    }
    ...
    function checkUserSignExists(targetUsername, name) {
      return $(
        `span#LZTUpCustomSign.${name}[data-username="${targetUsername}"]`,
      ).length;
    }
    ...
    async function loadUserSigns() {
      let userSigns;
      try {
        userSigns = await requestJSON(api_endpoints.getUserSigns);
      } catch {}
    ...
    if (isContestsDBInited) {
      ...
      if (
        dbContestsData.contestsTen === 1 ||
        dbContestsData.contestsAll === 1
      ) {
      ...
      dbContestsData.contestsInfoTop === 1
        ? await contestThreadBlockMove(true)
        : null;
      dbContestsData.contestsBtnTopInBlock === 1
        ? await contestsBtnInBlockMove(true)
        : null;
      dbContestsData.contestsHideTags === 1
        ? await contestsTagsVisibility(true)
        : null;
      dbContestsData.contestsAutoClose === 1
        ? await contestsAutoCloseFunc(true)
        : null;
      dbContestsData.contestsRmContent === 1
        ? await contestsContentVisibility(true)
        : null;
      dbContestsData.contestsShowWinChance === 1
    ...
    if (isUsersDBInited) {
      ...
      dbUsersData.showUseridInProfile === 1
        ? await addUserIdInProfileInfo()
        : null;
      dbUsersData.showFullRegInProfile === 1
        ? await editUserRegInProfileInfo(true)
        : null;
      dbUsersData.disableShowTyping === 1
        ? await disableShowTypingExecute()
        : null;
    }
    ...
    if (isAppearDBInited) {
      ...
      dbAppearData.hideUnreadArticleCircle === 1
      ...
      if (dbAppearData.forumLogo > 0) {
        $.ajax({
          ...
          success: logo =>
            typeof logo === "object" &&
            logo.hasOwnProperty("uid") &&
            logo.hasOwnProperty("css")
              ? updateSiteLogo("main", logo.css)
              : undefined,
          error: err =>
            Logger.error(
              `LZT Upgrade: Не удалось загрузить логотип форума `,
              err,
            ),
        });
      }
      if (dbAppearData.marketLogo > 0) {
        $.ajax({
          ...
          error: err =>
            Logger.error(
              `LZT Upgrade: Не удалось загрузить логотип маркета `,
              err,
            ),
        });
      }
      ...
      dbAppearData.hideCounterAlerts === 1
        ? await counterVisibility("alerts", true)
        : null;
      dbAppearData.hideCounterConversations === 1
        ? await counterVisibility("conversations", true)
        : null;
      dbAppearData.hideCounterAlerts === 1 ||
      dbAppearData.hideCounterConversations === 1
      ...
      dbAppearData.showPollsResults === 1
    ...
    if (MenuResult === true) {
      ...
      $(document).on("click", "#LZTUpSaveUniqueDB", async function () {
        ...
        if (nickStyleNew.length < 1501) {
          ...
          Logger.log(
            "Не удалось сохранить стиль ника. Стиль ника не должен превышать 1500 символов!",
          );
        }
        ...
        if (bannerStyleNew.length < 1501) {
          ...
          Logger.log(
            "Не удалось сохранить стиль лычки. Стиль лычки не должен превышать 1500 символов!",
          );
        }
        ...
        if (bannerTextNew.length < 25) {
          ...
          Logger.log(
            "Не удалось сохранить текст в лычке. Текст в лычке не должен превышать 24 символов!",
          );
        }
        ...
        if (badgeIconNew.length < 3001) {
          ...
          registerAlert(
            "Иконка на аватарке не должна превышать 3000 символов!",
          );
          Logger.log(
            "Не удалось сохранить иконку на аватарке. Иконка на аватарке не должен превышать 3000 символов!",
          );
        }
        ...
        if (badgeTextNew.length < 25) {
          ...
          registerAlert(
            "Текст в иконке аватарки не должен превышать 24 символов!",
          );
          Logger.log(
            "Не удалось сохранить текст в иконке аватарки. Текст в иконке аватарки не должен превышать 24 символов!",
          );
        }
        ...
        $(document).on(
          ...
          async function () {
            ...
            if (typeof uniqueData.noticesMarks === "string") {
              ...
              if (
                usedNoticesMarks.find(
                  noticeId => noticeId === String(notice.id),
                )
              ) {
                let noticeIndex = usedNoticesMarks.indexOf(String(notice.id));
                noticeIndex !== -1
                  ? usedNoticesMarks.splice(noticeIndex, 1)
                  : null;
              } else {
            ...
            await uniqueStyleDB.update({
              noticesMarks: usedNoticesMarks.join(),
            });
        ...
        if (badgeIconNew.length < 3001) {
          ...
          if (badgeIconNew.length) {
            ...
            if (badgeIconNew.startsWith(".")) {
              ...
              badge.append(
                `<div class="customUniqIcon">${badgeIconNew
                  .replaceAll(/<[script|style]*>/gi, "<!--")
                  .replaceAll(/<\/[script|style]*>/gi, "-->")}</div>`,
              );
        ...
        async function () {
          ...
          if (profileBackgroundNew.length) {
            previewContainer.css(
              "background-image",
              `linear-gradient(rgba(54, 54, 54, 0.85), rgba(54, 54, 54, 0.85)), url(${profileBackgroundNew})`,
            );
          } else {
      ...
      $(document).on("click", "#disable_show_typing", async function () {
        $("#disable_show_typing")[0].checked
          ? (await usersDB.update({ disableShowTyping: 1 }),
            registerAlert(
              "Отправка информации о наборе сообщения отключена",
              5000,
            ),
            await sleep(500),
            window.location.reload())
          : (await usersDB.update({ disableShowTyping: 0 }),
            registerAlert(
              "Отправка информации о наборе сообщения включена",
              5000,
            ),
            await sleep(500),
            window.location.reload());
      ...
      $(document).on("click", "#hide_counter_alerts", async function () {
        $("#hide_counter_alerts")[0].checked
          ...
          : ((dbAppearData =
            ...
            dbAppearData.hideCounterAlerts === 0 &&
            dbAppearData.hideCounterConversations === 0
              ? counterMutation(false)
              : null,
      ...
      $(document).on("click", "#hide_onlyfans", async function () {
        $("#hide_onlyfans")[0].checked
          ? (await appearDB.update({ hideOnlyfans: 1 }),
            registerAlert(
              "Включено скрытие тега Onlyfans. Выполняю перезагрузку страницы...",
              5000,
            ),
            await sleep(500),
            window.location.reload())
          : (await appearDB.update({ hideOnlyfans: 0 }),
            registerAlert(
              "Скрытие тега Onlyfans отключено. Выполняю перезагрузку страницы...",
              5000,
            ),
            await sleep(500),
            window.location.reload());
      ...
      reportButtonsList.forEach(btn => {
        $(document).on("click", `#set_${btn.id}_reportbtn`, async function () {
          ...
          $(`#set_${btn.id}_reportbtn`)[0].checked
            ? await addReportBtnInPosts(btn.name, btn.reason)
            : await removeReportBtnInPosts(btn.name);
        });
      });
